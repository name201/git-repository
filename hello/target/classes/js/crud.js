...
$(function() {
	...
	$('#btnSave1').bind('click', function() {
		// 做表单字段验证，当所有字段都有效的时候返回true。该方法使用validatebox(验证框)插件。
		var selected = $('#grid').datagrid('getSelected');
		var isValid = $('#editForm1').form('validate');
		var json = eval('('+isValid+')');
		if (isValid == false) {
			return;
		}
		var formData = $('#editForm1').serializeJSON();

		$.ajax({
			url : "/user/upUser?id="+selected.id,
			data : formData,
			dataType : 'json',
			type : 'post',
			success : function(rtn) {
...
function del(selected) {
	$.messager.confirm("确认", "确认要删除吗？", function(yes) {
		...
		if (yes) {
			$.ajax({
				url : "/user/delegateName?id="+selected.id,
				dataType : 'json',
				type : 'post',
				success : function(rtn) {
